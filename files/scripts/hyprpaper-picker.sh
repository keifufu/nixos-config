...
if [[ -n "$image" ]]; then
  ...
  input="$SNOWFLAKE_WALLDIR/$image"
  output="$HOME/wall.png"

  magick -size $(identify -format "%wx%h" "$input") xc:black -fill "rgba(0,0,0,$opacity_fraction)" -draw "rectangle 0,0 $(identify -format "%w,%h" "$input")" overlay.png
